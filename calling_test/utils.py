...
async def handle_voice_agent(
    ...
    async def on_client_disconnected(transport, client):
        ...
        # Trigger Postcall Scripts
        try:
            from db import save_transcript_to_cosmos
            from postcall.main import process_transcript_and_update_json
            
            # 1. Fetch the completed conversation history straight out of Pipecat's memory context!
            full_transcript = context.get_messages()
            
            # 2. Inject it into the current user's payload
            user_data["transcript"] = full_transcript
            
            print("PIPING TRANSCRIPT TO LANGCHAIN POSTCALL AGENT...")
            
            # 3. TRIGGER OUR NEW SYSTEM (Asynchronously!)
            final_json = await process_transcript_and_update_json(user_data)
            
            # 4. (Optional) Run your original Cosmos DB save script if you still need it
